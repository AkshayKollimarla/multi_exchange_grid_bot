...
export async function runOptionEntry({ instrument, qty, isCoinSettled, onLog, isCancelled }) {
  ...
  async function place(mid) {
    ...
    const data = await placeOrder({ instrument, qty: Math.abs(qty), direction: dir, price: mid, isMarket: false, postOnly: false });
  ...
  while (!filled) {
    if (cancelled()) { await cancelOrder(orderId).catch(() => {}); throw new Error("Cancelled by user"); }
    await sleep(POLL_MS);
    if (cancelled()) { await cancelOrder(orderId).catch(() => {}); throw new Error("Cancelled by user"); }
...
export async function runFuturesEntry({ instrument, qty, onLog }) {
  const dir = qty > 0 ? "buy" : "sell";
  const log = onLog || (() => {});
  log(`Placing futures MARKET ${dir} ${Math.abs(qty)}x ${instrument}`);
  const data = await placeOrder({ instrument, qty: Math.abs(qty), direction: dir, isMarket: true });
