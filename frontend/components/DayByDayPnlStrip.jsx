...
// dayByDayFlatPnl()/dayByDayMovePnl() in lib/optionsDerived.js and just
// render them here.
export default function DayByDayPnlStrip({
  rows,
  title = "📅 Day-by-Day PnL (theta decay, price held flat at today's level)",
  assumption = "price holds flat",
}) {
  if (!rows || rows.length < 2) return null;

  const startPositive = rows[0].pnl >= 0;
  const flipIdx = rows.findIndex((r, i) => i > 0 && (r.pnl >= 0) !== startPositive);
  // At-the-money options carry the most time value and it collapses fastest
  // right before expiry (theta accelerates hard in the final day) — so a
  // big last-day move here is normal Black-Scholes behavior, not a bug.
  const lastStepJump = rows.length > 1 ? Math.abs(rows[rows.length - 1].pnl - rows[rows.length - 2].pnl) : 0;
  const priorStepJump = rows.length > 2 ? Math.abs(rows[rows.length - 2].pnl - rows[rows.length - 3].pnl) : 0;
  const showCliffNote = rows.length > 2 && lastStepJump > Math.max(5, priorStepJump * 1.75);
      ...
      <div className="card-body">
        {flipIdx > 0 ? (
          <div style={{ fontSize: 12.5, marginBottom: 12, padding: "8px 12px", borderRadius: 8, background: startPositive ? "var(--red-soft)" : "var(--green-soft)", color: startPositive ? "var(--red-2)" : "var(--green-2)", fontWeight: 600 }}>
            If {assumption}, PnL flips {startPositive ? "negative" : "positive"} on{" "}
          ...
          <div style={{ fontSize: 12.5, marginBottom: 12, color: "var(--muted)" }}>
            {startPositive ? "Stays positive" : "Stays negative"} every day through expiry if {assumption}.
          </div>
        )}
        ...
        {showCliffNote && (
          <div style={{ fontSize: 11, marginTop: 10, color: "var(--muted)" }}>
            ℹ️ The jump on the last day is expected, not a bug — time value on an at-the-money option collapses fastest right before expiry, so the final day's move is naturally bigger than earlier days.
          </div>
        )}
      </div>
    </div>
